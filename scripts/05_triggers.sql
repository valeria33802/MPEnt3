...
BEGIN
   ...
   IF NEW.EMP_Nombre NOT REGEXP '^[A-Za-z]+$' THEN
       SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El nombre debe contener solo letras sin espacios ni números.';
   END IF;
   -- Validar que COM_Apellido contenga solo letras
   IF NEW.EMP_Apellido  NOT REGEXP '^[A-Za-z]+$' THEN
       SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El apellido debe contener solo letras sin espacios ni números.';
   END IF;
END //
